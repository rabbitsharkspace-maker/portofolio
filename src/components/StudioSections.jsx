...
export function Recognition({ who }) {
  ...
  return <section className={`recognition-ticket ${who ? 'personal-recognition' : ''}`}>
    <div className="recognition-stub"><span className="micro-label">{zh ? '外部评价' : 'JUDGED BY OTHERS'}</span><span className="award-flower" aria-hidden="true">✳</span><strong>{who ? (zh ? '得奖记录' : 'Awards.') : '2 / 6'}</strong><p>{who ? (zh ? '有公开战报可查' : 'Each one publicly listed.') : (zh ? '六个获奖席位，两个来自我们两人' : 'Six winning places. Two of them ours.')}</p><span className="ticket-barcode" aria-hidden="true" /></div>
    <div className="recognition-body"><p className="micro-label">{who ? people[who].name : 'JANE + JENNY'}</p><h2>{who ? (zh ? '一路留下的印记' : 'Collected along the way.') : (zh ? <>同一个两人工作室，<br /><em>拿下其中两个</em></> : <>Six winners that year.<br /><em>Two came from one desk.</em></>)}</h2><ul>{awards.map(a=><li key={a}>{a}</li>)}</ul>{!who && <p className="recognition-context">{recognition.path}</p>}<a className="quiet-link" href={recognition.source} target="_blank" rel="noreferrer">{zh ? 'Google AI Vibe-a-thon · 官方战报' : 'Google AI Vibe-a-thon · official recap'} ↗</a></div>
  </section>
}
...
export function AutomationReceipt() {
  ...
  return <section id="ticketing-proof" className="automation-scene"><div className="automation-story"><p className="micro-label">JENNY / {zh ? '社区智能工单' : 'COMMUNITY TICKETING'}</p><h2>{zh ? <>每天 100+ 条报修，<br /><em>没有人手工派单</em></> : <>100+ requests a day,<br /><em>nobody sorting them.</em></>}</h2><p>{ticketing[lang].what}</p><a href={`https://youtu.be/${ticketing.film.youtube}`} className="ink-button" target="_blank" rel="noreferrer">{zh ? '看它怎么跑' : 'Watch it run'} ↗</a></div><div className="automation-receipt"><p className="micro-label">RABBITSHARK / DAILY OPERATIONS</p><strong>100<span>+</span></strong><p>{zh ? '条报修 / 每天自动处理' : 'maintenance requests / handled daily'}</p><div className="receipt-rule" />{(zh ? ['收到报修','分类与优先级','派给对应人员'] : ['Request received','Classified & prioritised','Assigned to the right team']).map((label,i)=><div className="receipt-step" key={label}><span>0{i+1}</span><span>{label}</span><span>✓</span></div>)}<div className="receipt-rule" /><div className="receipt-total"><span>{zh ? '响应时间' : 'RESPONSE TIME'}</span><strong>{zh ? '小时 → 秒' : 'hours → seconds'}</strong></div><span className="receipt-signature">{zh ? '手工的部分，少了一块。—— Jenny' : 'a little less manual. — Jenny'}</span></div></section>
}

export function Services() {
  const { lang } = useLang()
  const zh = lang === 'zh'
  return <section className="service-folder"><div className="service-title"><p className="micro-label">{zh ? '我们能帮上什么' : 'WHAT WE CAN DO'}</p><h2>{zh ? <>你遇到的问题，<br /><em>是哪一种？</em></> : <>Which of these<br /><em>sounds like you?</em></>}</h2><span className="service-asterisk" aria-hidden="true">✳</span><p>{zh ? '选一条打开，里面有我们做过的同类项目' : 'Open one to see a project we have already done like it.'}</p></div><div className="service-sheets">{studio[lang].offers.map((o,i)=><details key={o.title} open={i === 0}><summary><span>0{i+1}</span><h3>{o.title}</h3><span className="notebook-plus" aria-hidden="true">＋</span></summary><div><p>{o.body}</p><a className="quiet-link" href={o.case === 'ticketing' ? '#ticketing-proof' : `#case-${o.case}`}>{o.proof} ↗</a></div></details>)}</div></section>
}
...
export function BackgroundFolder({ who }) {
  ...
  return <section className="background-folder"><div><p className="micro-label">{zh ? '履历' : 'BACKGROUND'}</p><h2>{zh ? <>读过的书，<br /><em>做过的事</em></> : <>Where I studied,<br /><em>and what I have done.</em></>}</h2><div className="background-stamp">RABBITSHARK<br /><strong>{who === 'jane' ? 'JANE' : 'JENNY'}</strong><br />ALWAYS LEARNING</div></div><div className="background-entries">{groups.map((kind,i)=><details key={kind} open={i === 0}><summary><span>0{i+1}</span><h3>{BG_KINDS[kind][lang]}</h3><span className="notebook-plus" aria-hidden="true">＋</span></summary><ul>{c.background.filter(b=>b.kind===kind).map(b=><li key={b.text}>{b.text}{b.note && <small>{b.note}</small>}</li>)}</ul></details>)}</div></section>
}

export function ContactLetter({ who }) {
  const { lang } = useLang()
  const zh = lang === 'zh'
  return <section id="contact" className="contact-letter"><div className="contact-invitation"><p className="micro-label">{zh ? '联系我们' : 'GET IN TOUCH'}</p><h2>{zh ? <>说说你的项目，<br /><em>我们 24 小时内回复</em></> : <>Tell us about your project.<br /><em>We reply within 24 hours.</em></>}</h2><p>{who ? people[who][lang].replyNote : (zh ? '还没想清楚也没关系，写下你现在遇到的情况就行' : 'It does not have to be a brief. Tell us what is happening and we will take it from there.')}</p><a className="contact-address" href={`mailto:${studio.email}`}>{studio.email} ↗</a><div className="contact-characters" aria-hidden="true"><img src={people.jane.art} alt="" loading="lazy" /><img src={people.jenny.art} alt="" loading="lazy" /><span>say hello.<br />make something.</span></div></div><div className="letter-paper"><div className="letter-heading"><span>{zh ? `致：${who ? people[who].short : 'Jane & Jenny'}` : `TO: ${who ? people[who].short : 'JANE & JENNY'}`}<small>RABBITSHARK STUDIO</small></span><div className="letter-stamp"><img src="/ip/logo.png" alt="RabbitShark logo" /><span>WITH CURIOSITY</span></div></div><ContactForm /></div></section>
}

export function StudioFooter() {
  const { lang } = useLang()
  return <footer className="studio-footer"><Link to="/" className="footer-brand"><img src="/ip/logo.png" alt="RabbitShark Studio" /><span>{lang === 'zh' ? '墨尔本的两人工作室，做 AI 产品、界面与自动化' : 'A two-person studio in Melbourne. AI products, interfaces and automation.'}</span></Link><div className="footer-links">{studio.socials.filter(s=>s.label!=='Email').map(s=><a key={s.label} href={s.url} target="_blank" rel="noreferrer">{s.label} ↗</a>)}</div><div className="footer-bottom"><span>© 2026 RabbitShark</span><span>Melbourne & everywhere / EN + 中文</span><a href="#main-content">{lang === 'zh' ? '回到顶部' : 'Back to the top'} ↑</a></div></footer>
}
