...
export default function StudioWorld({items,onOpen}){
  ...
  return <div ref={box} tabIndex={-1} className={`studio-world ${expanded?'world-expanded':''}`}>
    ...
    <div className="world-canvas" aria-label={zh?'可拖动旋转的双人工作室展岛':'A rotatable miniature studio island'}>
      ...
      {hello && <div className="world-speech" role="status">{hello==='jane'?(zh?'Jane：欢迎来到我的设计角':'Jane: welcome to my design corner.'):(zh?'Jenny：每个小机关，都有它的道理':'Jenny: a little method behind the magic.')}<button onClick={()=>setHello(null)} aria-label={zh?'关闭问候':'Close greeting'}>×</button></div>}
      <div className="world-instructions">{zh?'拖动转一转 · 点作品进去看 · 和桌上的两位打个招呼':'Drag to orbit · Tap a project · Say hello to the two of us'}</div>
    </div>
