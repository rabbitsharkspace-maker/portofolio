...
function Diorama({items,onOpen,animated,revision,lang}){
...
export default function StudioWorld({items,onOpen}){
  ...
  return <div ref={box} tabIndex={-1} className={`studio-world ${expanded?'world-expanded':''}`}>
    ...
    <div className="world-canvas" aria-label={zh?'可拖动旋转的双人工作室展岛':'A rotatable miniature studio island'}>
      {near ? <SceneBoundary fallback={fallback}><Canvas orthographic shadows dpr={[1,1.5]} camera={{position:[7,8,12],zoom:60,near:.1,far:100}} gl={{antialias:true,alpha:true}} frameloop={animated?'always':'demand'} fallback={fallback}><Diorama items={items} onOpen={index=>{box.current?.focus({preventScroll:true});onOpen(index,{currentTarget:box.current})}} animated={animated} revision={revision} lang={lang}/></Canvas></SceneBoundary> : fallback}
      <div className="world-instructions">{zh?'拖动转一转 · 点作品进去看':'Drag to orbit · Tap a project'}</div>
    </div>
    <div className="world-controls"><div>{items.map((item,i)=><button key={item.id} id={`case-${item.id}`} onClick={e=>onOpen(i,e)}><span>0{i+1}</span>{item[lang].name} ↗</button>)}</div><div><button onClick={()=>{setRevision(r=>r+1)}}>{zh?'回正':'Recenter'} ⟲</button><button aria-pressed={play} onClick={()=>setPlay(p=>!p)}>{play?(zh?'暂停动态':'Pause motion'):(zh?'恢复动态':'Play motion')}</button><button aria-pressed={expanded} onClick={()=>setExpanded(e=>!e)}>{expanded?(zh?'收起':'Smaller'):(zh?'放大逛':'Expand')} ⤢</button></div></div>
  </div>
}
