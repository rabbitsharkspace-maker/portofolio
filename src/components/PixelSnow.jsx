...
// Carrot silhouette in flake-local coords normalised to ~[-1,1].
// Returns 1.0 for the root body, 2.0 for the leafy top, 0.0 outside.
float carrotRegion(vec2 q) {
  // root: tapered wedge, wide at the top, tip pointing down
  float bodyTop = 0.35;
  float bodyBot = -0.95;
  float hw = 0.40 * clamp((q.y - bodyBot) / (bodyTop - bodyBot), 0.0, 1.0);
  if (q.y >= bodyBot && q.y <= bodyTop && abs(q.x) <= hw) return 1.0;
  // leaves: three little fronds fanning up from the shoulders
  float leafBot = bodyTop - 0.04;
  float leafTop = 0.98;
  if (q.y >= leafBot && q.y <= leafTop) {
    float up = (q.y - leafBot) / (leafTop - leafBot); // 0 at base -> 1 at tip
    float spread = 0.34 * (1.0 - up);                 // fronds narrow toward tips
    float x = abs(q.x);
    // centre frond + two side fronds
    if (x <= 0.06 * (1.0 - up * 0.5)) return 2.0;
    if (abs(x - spread) <= 0.06 * (1.0 - up * 0.5)) return 2.0;
  }
  return 0.0;
}
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        if (uVariant < 0.5) {
          ...
          // carrot: taller than wide, tip down, two-tone (root + leaves)
          float region = carrotRegion(vec2(testX, testY) / flakeSize);
          dist = region > 0.5 ? 0.0 : flakeSize * 4.0;
          flakeCol = region > 1.5 ? uLeafColor : uColor;
        }
        ...
        if (dist < flakeSize) {
          ...
          gl_FragColor = vec4(flakeCol * pow(vec3(intensity), vec3(uGamma)), 1.0);
          return;
        }
      }
    }
...
export default function PixelSnow({
  ...
  const variantValue = useMemo(() => {
    return variant === 'round' ? 1.0 : variant === 'snowflake' ? 2.0 : variant === 'carrot' ? 3.0 : 0.0;
  }, [variant]);
  ...
  const leafVector = useMemo(() => {
    const threeColor = new Color(leafColor);
    return new Vector3(threeColor.r, threeColor.g, threeColor.b);
  }, [leafColor]);
  ...
  useEffect(() => {
    ...
    const material = new ShaderMaterial({
      ...
      uniforms: {
        ...
        uLeafColor: { value: leafVector.clone() },
  ...
  useEffect(() => {
    ...
    material.uniforms.uLeafColor.value.copy(leafVector);
  }, [
    flakeSize,
    minFlakeSize,
    pixelResolution,
