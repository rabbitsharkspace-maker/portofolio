...
const ModelInner = ({
  ...
  useLayoutEffect(() => {
    ...
    // measure at scale 1 so a second pass (StrictMode remounts this) reads the
    // same box instead of one that already carries the last fit
    g.scale.setScalar(1);
    ...
    /*
     * Centre the content inside the group rather than moving the group: a group's
     * position is applied *before* its scale, so offsetting `g` by the raw box
     * centre under-corrects by the scale factor — and the pivot read back a few
     * lines down then treats that leftover as a real offset and shifts the whole
     * rig by it again. A model whose bounds sit off the origin (anything that
     * stands on the floor rather than around it) lands clean out of frame.
     * Shifting the child leaves the group's origin on the model's centre, which
     * is what the pivot, the parallax and the orbit target all assume.
     */
    content.position.sub(sphere.center);
    g.scale.setScalar(s);
    ...
    window.__mv = { outer: outer.current, inner: g, content, camera };
