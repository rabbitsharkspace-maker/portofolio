...
export default function ContactForm() {
  ...
  return <div className={`letter-interaction letter-phase-${phase}`}>
    {phase!=='packed' && <form className="letter-form tear-sheet" noValidate onSubmit={send}>
      <fieldset disabled={phase!=='editing'}><input ref={firstField} className="letter-field" value={form.name} onChange={update('name')} placeholder={zh?'怎么称呼你（选填）':'Your name (optional)'} aria-label={zh?'怎么称呼你（选填）':'Your name (optional)'} autoComplete="name" /><textarea className="letter-field letter-message" value={form.problem} onChange={update('problem')} placeholder={zh?'你想做什么，或者卡在哪里（选填）':'What you want to build, or where you are stuck (optional).'} aria-label={zh?'信件内容（选填）':'Your note (optional)'} rows={4} /><input className="letter-field" type="text" inputMode="email" value={form.email} onChange={update('email')} placeholder={zh?'回信地址（选填）':'Reply address (optional)'} aria-label={zh?'回信地址（选填）':'Reply address (optional)'} autoComplete="email" /><div className="letter-submit-row"><button type="submit" className="letter-tear-button">{zh?'写好了，打开邮件':'Write it and open my email'}<span aria-hidden="true">↗</span></button><span>{zh?'不会直接发送':'nothing is sent yet.'}</span></div></fieldset>
      <p className="letter-compose-note">{zh?'点击后会打开你的邮件应用，发送前你可以再改':'This opens your email app. You can edit it before sending.'}</p>
    </form>}
    ...
    {phase==='packed' && <div className="letter-packed-message" ref={status} tabIndex={-1} role="status"><h3>{zh?'邮件已经准备好了':'Your email is ready.'}</h3><p>{zh?'请到邮件应用里点发送。这个页面看不到你有没有发出':'Now press send in your email app. This page cannot tell whether you did.'}</p><a href={mailto} className="quiet-link">{zh?'再次打开邮件应用':'Open email app again'} ↗</a><button onClick={()=>{setPhase('editing');setTimeout(()=>firstField.current?.focus(),0)}}>{zh?'返回继续修改':'Go back and edit'} ↶</button></div>}
