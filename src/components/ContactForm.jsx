 ...
 * above it changes.
 */
export default function ContactForm({ accent = "var(--accent)" }) {
  const { lang } = useLang()
  const T = ui[lang]
  ...
  return (
    <form onSubmit={send} noValidate className="letter-form">
      <div className="space-y-3">
        ...
        <textarea
          ...
          rows={4}
          className="letter-field letter-message"
          style={field}
        />
        <input
          value={form.email}
      ...
      <div className="mt-5 flex flex-wrap items-center gap-4">
        ...
        {error && (
          <span role="alert" className="text-[13px]" style={{ color: "#b3261e" }}>
            {error}
          </span>
        )}
        {sent && !error && (
          <span role="status" className="text-[13px]" style={{ color: "var(--dim)" }}>
            {T.formSent}
          </span>
        )}
      </div>
