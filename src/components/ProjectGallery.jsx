  ...
  return <div className={`project-gallery gallery-mode-${mode}`}>
    ...
    <dialog ref={dialog} className="gallery-dialog" aria-label={zh?'作品展台':'Project exhibition'} onClose={()=>{setSelected(null);opener.current?.focus()}} onClick={e=>{if(e.target===e.currentTarget)close()}} onKeyDown={e=>{
      ...
      <AnimatePresence mode="wait" initial={false}>{item && <motion.div key={item.id} className={`exhibition-content exhibition-${item.id}`} initial={reduced?false:{opacity:0}} animate={{opacity:1}} exit={reduced?{}:{opacity:0}} transition={{duration:.2}}>
        ...
        {item.id === 'serene' ? <SereneStory scenes={item.scenes} root={dialog} /> : <CaseScroller scenes={item.scenes} mark={item.mark} root={dialog} />}
        <section className="exhibition-notes"><div className="exhibition-notes-heading"><h3>{zh?'近一点，看细节':'A closer look.'}</h3><div role="group" aria-label={zh?'筛选界面解读':'Filter interface notes'}>{['all','jane','jenny'].filter(who=>who==='all'||item.notes.some(n=>n.by===who)).map(who=><button key={who} aria-pressed={notes===who} onClick={()=>setNotes(who)}>{who==='all'?(zh?'全部':'All'):who==='jane'?'Jane':'Jenny'}</button>)}</div></div>{item.notesNote && <p className="notes-ui-language">{item.notesNote[lang]}</p>}<div className="exhibition-note-grid">{item.notes.filter(n=>notes==='all'||notes===n.by).map((n,i)=><details key={n.en.q} className={`by-${n.by}${n.shot ? ' has-shot' : ''}`}><summary><span>0{i+1} / {n.by==='jane'?'Jane':'Jenny'}</span><h4>{n[lang].q}</h4><b aria-hidden="true">＋</b></summary><div className="note-body"><p>{n[lang].a}</p>{n.shot && (()=>{ const src = typeof n.shot === 'string' ? n.shot : n.shot[lang]; return <figure className="note-shot"><img src={src} alt={`${item[lang].name} — ${n[lang].q}`} loading="lazy" />{n[lang].shotNote && <figcaption>{n[lang].shotNote}</figcaption>}<a className="note-shot-full" href={src} target="_blank" rel="noreferrer">{zh ? '看大图 ↗' : 'View full size ↗'}</a></figure> })()}</div></details>)}</div></section>
